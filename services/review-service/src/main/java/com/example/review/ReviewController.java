...
public class ReviewController {
    ...
    public Map<String, Object> summary() {
        ...
        List<RatingDistributionPoint> ratingDistribution = new ArrayList<>();
        for (int rating = 5; rating >= 1; rating--) {
            Integer count = jdbc.sql("select count(*) from reviews where rating = ?").param(rating).query(Integer.class).single();
            ratingDistribution.add(new RatingDistributionPoint(rating, count == null ? 0 : count));
        }
        return Map.of(
    ...
    record RatingDistributionPoint(int rating, int count) {}
}
